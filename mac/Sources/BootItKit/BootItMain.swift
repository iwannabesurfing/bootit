...
/// What the executable target's `@main` calls.
///
/// The entry point itself lives in `Sources/BootIt`, which is now a thin
/// executable over this library. That split is not tidiness: Xcode resolves a
/// preview host from the scheme, and any scheme containing an executable target
/// makes that executable the host — which cannot carry `ENABLE_PREVIEWS`, so
/// every `#Preview` in the package failed with "the executable target "BootIt"
/// needs the build setting …", including previews for files in a *library*
/// target. Views in a library **plus** a library product, so Xcode offers a
/// scheme with no executable in it, is what actually renders. Verified both
/// ways on Xcode 26.6 before this was moved.
///
/// Normally launches the SwiftUI app; with `--selftest` it runs a headless
/// check of the Microsoft download API (handy for diagnosing the notoriously
/// flaky catalogue endpoints without a USB drive).
public enum BootItMain {
