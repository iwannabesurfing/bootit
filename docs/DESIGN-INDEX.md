...
|---|---|---|---|
| **macOS copy progress** — what the ring shows during `createinstallmedia`'s stretch, and why it claims no percentage. ⚠ **The synthesis rests on one premise measured false on 2026-08-05:** the copy is not silent — `Copying to disk: 0%…100%` is emitted, block-buffered down a pipe and delivered in one batch at ~90%. Read [the 2026-08-05 session log](../.claude/session-log.md) beside the synthesis; the decision it reached still stands, its stated reason does not. | [`copy-progress-reporting-trimodel-synthesis.md`](research/copy-progress-reporting-trimodel-synthesis.md) | [`copy-progress-measurements.md`](research/copy-progress-measurements.md) · [brief](research/copy-progress-reporting-trimodel-brief.md) · legs: [Claude](research/copy-progress-reporting-claude-leg.md), [ChatGPT](research/copy-progress-reporting-chatgpt-leg.md), [Gemini](research/copy-progress-reporting-gemini-leg.md) | `CopyProgressModel` · `CopyRing` · `InstallMediaProgress` · `RecordedRunTests` + `EveryRecordedRunTests` (two traces) |
